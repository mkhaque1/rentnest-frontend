...
type NavItem = 'overview' | 'users' | 'properties' | 'rentals';
  ...
  return (
    <div className='min-h-screen bg-background flex'>
      ...
      <aside className='hidden lg:flex flex-col w-64 border-r border-border bg-card shrink-0'>
        <Link href='/' className='flex items-center gap-2.5 px-6 h-16 border-b border-border shrink-0'>
          <ShieldAlert className='h-5 w-5 text-destructive' />
          <span className='text-heading text-lg tracking-tight'>Admin</span>
        </Link>
      ...
      <div className='flex-1 flex flex-col min-h-screen overflow-hidden'>
        ...
        <header className='h-16 border-b border-border bg-card/50 backdrop-blur-sm flex items-center justify-between px-6 lg:px-10 shrink-0'>
          ...
          <div className='flex items-center gap-3'>
            <button className='h-9 w-9 rounded-xl bg-secondary border border-border flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors'>
...
function OverviewTab({ users, props, rentals, usersLoading, propsLoading, rentalsLoading, onNav }: {
  users: ReturnType<typeof useAdminUsers>['data'] & object[];
  props: ReturnType<typeof useAdminProperties>['data'] & object[];
  rentals: ReturnType<typeof useAdminRentals>['data'] & object[];
      ...
      <div className='grid grid-cols-2 lg:grid-cols-4 gap-4'>
        {(usersLoading || propsLoading || rentalsLoading)
          ...
          : stats.map(({ label, value, icon: Icon, color, bg }) => (
            <div key={label} className='rounded-2xl border border-border bg-card surface-edge p-5 space-y-3'>
              ...
              {users.map((u, i) => (
                <div key={u.id} className={cn('flex items-center justify-between px-5 py-4', i < users.length - 1 && 'border-b border-border')}>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    {u.role !== 'ADMIN' && (
                      <Button size='sm' variant='outline' className='rounded-xl h-7 text-xs gap-1' onClick={() => onToggleBan(u.id, u.status)} disabled={isPending}>
                        {u.status === 'BANNED'
  ...
  return (
    <div className='space-y-6'>
      ...
      {isLoading
        ? Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className='h-16 rounded-2xl mb-3' />)
        : props.length === 0
          ? <EmptyCard message='No properties yet.' />
          : (
            <div className='rounded-2xl border border-border bg-card surface-edge overflow-hidden'>
              {props.map((p, i) => (
  ...
  return (
    <div className='space-y-6'>
      ...
      {isLoading
        ? Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className='h-16 rounded-2xl mb-3' />)
        : rentals.length === 0
          ? <EmptyCard message='No rental requests yet.' />
          : (
            <div className='rounded-2xl border border-border bg-card surface-edge overflow-hidden'>
              {rentals.map((r, i) => (
